...
const MODAL_CLASSES = "bg-card border-zinc-700 !w-[98vw] !max-w-[98vw] md:!w-[92vw] md:!max-w-[92vw] lg:!w-[88vw] lg:!max-w-[88vw] xl:!w-[82vw] xl:!max-w-[82vw] 2xl:!w-[75vw] 2xl:!max-w-[75vw] max-h-[92vh] overflow-y-auto p-4 md:p-8";
...
export function ClienteViewModal({ isOpen, onClose, cliente, onEdit }) {
            ...
            <AlertDialogContent className={MODAL_CLASSES}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 py-6">
                    <div className="flex flex-col gap-3">
                        ...
                        <InfoBox label={cliente.cnpj ? "CNPJ" : "CPF"} value={cliente.cpfCnpj} />
                    </div>
                    <div className="flex flex-col gap-3">
                        <p className="text-orange-400 text-xs tracking-[0.3em] font-semibold uppercase">Contato</p>
                        <InfoBox label="E-mail" value={cliente.email} />
                        <InfoBox label="Telefone 1" value={cliente.celular1} />
                        {cliente.celular2 && <InfoBox label="Telefone 2" value={cliente.celular2} />}
                    </div>
                    <div className="flex flex-col gap-3">
                        <p className="text-orange-400 text-xs tracking-[0.3em] font-semibold uppercase">Endereço</p>
                        <InfoBox label="Logradouro" value={cliente.logradouro} />
                        <InfoBox label="Número" value={cliente.numImovel} />
                        <InfoBox label="Complemento" value={cliente.complemento} />
                        <InfoBox label="Bairro" value={cliente.bairro} />
                        <InfoBox label="Município/UF" value={`${cliente.municipio || ''} - ${cliente.uf || ''}`} />
                        <InfoBox label="CEP" value={cliente.cep} />
                    </div>
                </div>

                {/* <-- Rodapé atualizado com os botões alinhados */}
                <AlertDialogFooter className="pt-4 mt-4 border-t border-zinc-700 bg-card flex sm:justify-end items-center gap-3">
